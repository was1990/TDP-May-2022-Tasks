...
describe("check the seacrh functionality in google", function () {
  ...
  //tests
  it("Go to google and search for formula1", async () => {
    driver.get("http://www.google.co.uk");
    driver.findElement(By.id("L2AGLb")).click();
    await driver.findElement(By.name("q")).sendKeys("formula1", Key.ENTER);
    const val = await driver.wait(driver.getTitle(), 1000);

    assert.equal(val, "formula1 - Google Search");
  });
});
...
describe("Testing Bing", function () {
  ...
  it("Should search for skysports and print this to the screen", async function () {
    // Arrange
    ...
    await searchBar.sendKeys("skysports", Key.RETURN);
    ...
    assert.equal(searchText, "skysports");
  });
